...
OUT_DIR = ROOT / "工况" / "flac3d_support_search_stage3_cases"
...
def save_commands(case_id: str) -> dict[str, str]:
    keys = [
        "SAVE_INIT_COMMAND",
        "SAVE_ROADWAY_COMMAND",
        "SAVE_STEP01_COMMAND",
        "SAVE_STEP02_COMMAND",
        "SAVE_STEP03_COMMAND",
        "SAVE_STEP04_COMMAND",
    ...
    return {key: f"; {key} skipped by save-mode summary-only" for key in keys}
...
def build_case_mapping(
    ...
    support_area = 20.0 * face_width
    p_s = support_pressure * support_area

    case_id = f"s3_g{geo_no:03d}_p{pressure_no:02d}"
    ...
    mapping.update(save_commands(case_id))
    return mapping
...
def write_batch_files(out_dir: Path, dat_files: list[str], chunk_size: int) -> None:
    ...
    (out_dir / "batch_run_stage3_all.dat").write_text("\n".join(all_lines) + "\n", encoding="utf-8")
    ...
    for start in range(0, len(dat_files), chunk_size):
        ...
        batch_name = f"batch_run_stage3_{start + 1:03d}_{end:03d}.dat"
        (out_dir / batch_name).write_text("\n".join(lines) + "\n", encoding="utf-8")


def main() -> None:
    parser = argparse.ArgumentParser()
    parser.add_argument("--n-geo", type=int, default=60)
    ...
    support_levels_mpa = [0.5, 0.8, 1.1, 1.4, 1.7, 2.0, 2.3, 2.6]
    ...
    for geo_no, sample in enumerate(geo_samples, start=1):
        for pressure_no, support_pressure_mpa in enumerate(support_levels_mpa, start=1):
            mapping = build_case_mapping(geo_no, pressure_no, sample, support_pressure_mpa)
    ...
    with (OUT_DIR / "cases_stage3_support_search.csv").open("w", newline="", encoding="utf-8") as f:
        writer = csv.DictWriter(f, fieldnames=list(rows[0]))
        writer.writeheader()
        writer.writerows(rows)

    write_batch_files(OUT_DIR, dat_files, args.chunk_size)
    print(f"Generated {len(rows)} FLAC3D stage-3 support-search cases in {OUT_DIR}")
